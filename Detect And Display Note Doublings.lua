           ...
           if qn> qn_left then 
             ...
             if ms:len() == 3 and (midi == 9 or midi == 8) then 
               ...
               if pitch<=topvisiblepitch and pitch> ME_BottomPitch then 
                 ...
                 index0 = tostring(pixelpos_x).." "..tostring(pixelpos_y) --These indexes are easy to work with
                 if midi==9 then  
                    contemporary_table[pixelpos_y] =  pixelpos_x  
                    if count[index0]   then 
                       count[index0] = count[index0] + 1 else count[index0] =1 
                    end 
                 end 
                 -- attaching noteoff to noteon 
                 if midi==8 then  
                    if contemporary_table[pixelpos_y] then 
                       notelength = pixelpos_x - contemporary_table[pixelpos_y] 
                       index0 = tostring(contemporary_table[pixelpos_y] ).." "..tostring(pixelpos_y) -- index corresponding to the noteon
...
function draw() 
    for u,v in pairs(count) do 
      _x, _y = u:match("(%d+).-(%d+)") -- Splits up the index string
